...
// PeSTO evaluation tables (Ronald Friederich)

public static class Psqt
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int S(int mg, int eg) => (eg << 16) + mg;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int Mg(int packed) => (short)(uint)packed;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int Eg(int packed) => (short)((uint)(packed + 0x8000) >> 16);

    private static readonly int[] MgValue = [82, 337, 365, 477, 1025, 0];
    private static readonly int[] EgValue = [94, 281, 297, 512, 936, 0];

    public static readonly int[] PhaseInc = [0, 1, 1, 2, 4, 0];
    ...
    private static int[] BuildWhitePov()
    {
        ...
        for (int pt = 0; pt < 6; pt++)
        {
            ...
            {
                int white = S(MgValue[pt] + MgPst[pt][sq ^ 56], EgValue[pt] + EgPst[pt][sq ^ 56]);
                int black = S(MgValue[pt] + MgPst[pt][sq], EgValue[pt] + EgPst[pt][sq]);
                t[w | sq] = white;
                t[b | sq] = -black;
            }
        }
